<div wire:init="$refresh">
    ...
    <div class="card no-print">
        ...
        <div class="mat-card-header header-pink-gradient">
            ...
            <p id="cardHeaderSubtitle">Generate exam admit cards for selected class, section and exam.</p>
        </div>
                ...
                <div class="table-responsive">
                    <table class="mat-table">
                        ...
                        <tbody>
                            ...
                            <tr wire:key="admit-student-row-{{ $student->student_id }}">
                                <td class="check-col">
                                        <input type="checkbox"
                                            class="red-check"
        ...
        @else
            <div class="gen-empty">
                ...
                <p>Please select Class, Exam and Template, then click <strong>Filter</strong> to load students.</p>
            </div>
        @endif
    </div>

    @php
        // FIX: card size must come from the selected template's card_width/card_height,
        // not be hardcoded — mirrors the Student ID Card fix.
        $tmplW = $selectedTemplate?->card_width ?: '210mm';
        $tmplH = $selectedTemplate?->card_height ?: '148mm';
    @endphp
                ...
                <div class="modal-body">
                    <div style="display:flex;flex-wrap:wrap;justify-content:flex-start;gap:16px;">
                        ...
                        @php
                            ...
                            $schedules = json_decode($card['exam_schedules'] ?? '[]', true) ?: [];
                        @endphp
                        <div class="id-card-print" style="display: flex;flex-direction: column; width:{{ $tmplW }};height:{{ $tmplH }};background:{{ $tmpl?->background_color ?? '#fff' }};color:{{ $tmpl?->text_color ?? '#000' }};">
                            {{-- Header Band --}}
                            <div style="background:{{ $tmpl?->accent_color ?? '#007bff' }};padding:12px 14px;text-align:center;">
                                ...
                                <div style="font-size:.62rem;margin-top:2px;letter-spacing:.05em;">ADMIT CARD</div>
                            </div>
                                ...
                                <div style="flex:1;min-width:0;">
                                    ...
                                    <table style="font-size:.68rem;width:100%;border-collapse:collapse;color:{{ $tmpl?->text_color ?? '#000' }};">
                                        ...
                                        <tr>
                                            ...
                                            <td style="font-weight:500;">{{ $card['class'] }} {{ !empty($card['section']) ? '- '.$card['section'] : '' }}</td>
                                        </tr>
                                        @endif
                                ...
                                <table style="width:100%;border-collapse:collapse;font-size:.65rem;">
                                    <thead>
                                        <tr style="background:rgba(0,0,0,.05);">
                                            ...
                                            <th style="padding:3px 6px;text-align:left;">Marks</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @foreach($schedules as $row)
                                        <tr>
                                            <td style="padding:3px 6px;border-top:1px solid rgba(0,0,0,.08);">{{ $row['subject'] ?? '' }}</td>
                                            <td style="padding:3px 6px;border-top:1px solid rgba(0,0,0,.08);">{{ !empty($row['exam_date']) ? \Carbon\Carbon::parse($row['exam_date'])->format('d M Y') : '' }}</td>
                                            <td style="padding:3px 6px;border-top:1px solid rgba(0,0,0,.08);">{{ $row['start_time'] ?? '' }}</td>
                                            <td style="padding:3px 6px;border-top:1px solid rgba(0,0,0,.08);">{{ $row['full_marks'] ?? '' }}</td>
                                        </tr>
                                        @endforeach
                                    </tbody>
                                </table>
                            ...
                            <div style="padding:4px 14px 0;display:flex;align-items:center;justify-content:center;gap:10px;margin-top: auto;">
    ...
    <div class="print-section">
        <style>
            @media print {
                /* FIX: force browsers to print backgrounds/gradients/colors (Chrome, Edge, Firefox, Safari) */
                html, body, * {
        ...
        <div class="print-cards-grid">
            ...
            <div class="id-card-print" style="width:{{ $tmplW }};height:{{ $tmplH }};background:{{ $tmpl?->background_color ?? '#fff' }};color:{{ $tmpl?->text_color ?? '#000' }};">
                <div style="background:{{ $tmpl?->accent_color ?? '#007bff' }};padding:12px 14px;text-align:center;">
                    ...
                    <div style="color:rgba(255,255,255,.7);font-size:.62rem;">ADMIT CARD</div>
                </div>
...
<script>
    ...
    function renderAdmitCardCodes(root) {
        if (!root) return;
    ...
    document.addEventListener('livewire:initialized', () => {
        renderAdmitCardCodes(document);

        Livewire.hook('morph.updated', ({ el }) => {
            setTimeout(() => {
                renderAdmitCardCodes(el);
            }, 0);
        });
    });
</script>
