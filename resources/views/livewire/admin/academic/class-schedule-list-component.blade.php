<div>
    ...
    <div class="card">
        ...
        <div id="classSchedulePrintable">
            ...
            {{-- Print-only header (class/section name), shown only inside print window --}}
            <div class="print-only-header" style="display:none">
                <h2>Class Schedule</h2>
                @if($filterClass)
                    <p>
                        Class: {{ optional($classes->firstWhere('id', (int) $filterClass))->name }}
                        @if($filterSection && $filterSection !== 'all')
                            &middot; Section: {{ optional($sections->firstWhere('id', (int) $filterSection))->name }}
                        @elseif($filterSection === 'all')
                            &middot; Section: All Section
                        @endif
                    </p>
                @endif
            </div>
                        ...
                        <tr>
                            ...
                            <td class="sched-td-cell {{ $item ? 'sched-c--science' : 'sched-c--empty' }}">
                                <div class="sched-cell-in">
                                    @if($item)
                                        <div>
                                            <span class="sched-subj-name">{{ $item['subject_name'] }}</span>
                                        </div>
                                        <div>
                                            <span class="sched-tchr-name">{{ $item['teacher_name'] }}</span>
            ...
            <div class="d-flex justify-content-end gap-2 px-5 pb-4 no-print">
                <button type="button" class="btn btn-primary" onclick="printClassSchedule()">
                    <span>
...
<script>
    ...
    function printClassSchedule() {
        const printableEl = document.getElementById('classSchedulePrintable');
        ...
        if (!gridWrap) {
            alert('Print korar moto kono schedule pawa jayni. Age Class/Section select kore "Filter" chaap din.');
            return;
        }

        const printHeader = printableEl.querySelector('.print-only-header');
        const headerHtml = printHeader ? printHeader.innerHTML : '<h2>Class Schedule</h2>';
