<div>
    <div class="card tpl-wizard">
        ...
        <div class="mat-card-header header-primary-gradient">
            ...
            <p>Update your certificate in {{ $totalSteps }} simple steps</p>
        </div>
        ...
        <div class="form-section {{ $step == 2 ? '' : 'd-none' }}">
            ...
            <div class="tpl-note mt-3">
                ...
                No worries — you can skip this and edit your own content in Step 4 instead.
            </div>
        </div>
        ...
        <div class="form-section {{ $step == 3 ? '' : 'd-none' }}">
            ...
            <div class="section-title mb-2">Upload Images <span class="text-muted fw-normal" style="font-size:.75rem">(optional — leave blank to keep the current image)</span></div>
            <div class="row g-4">
                ...
                <div class="col-md-4 col-6">
                    ...
                    <label class="photo-upload-box" style="cursor:pointer">
                        @if($logo_image)
                            ...
                            <span class="lbl">New image selected ✓</span>
                        @elseif($template->logo_image)
                            <img src="{{ asset($template->logo_image) }}" class="tpl-upload-preview" alt="Current logo">
                            <span class="lbl">Click to replace logo</span>
                        @else
                ...
                <div class="col-md-4 col-6">
                    ...
                    <label class="photo-upload-box" style="cursor:pointer">
                        @if($signature_image)
                            ...
                            <span class="lbl">New image selected ✓</span>
                        @elseif($template->signature_image)
                            <img src="{{ asset($template->signature_image) }}" class="tpl-upload-preview" alt="Current signature">
                            <span class="lbl">Click to replace signature</span>
                        @else
                ...
                <div class="col-md-4 col-12">
                    ...
                    <label class="photo-upload-box" style="cursor:pointer">
                        ...
                        @elseif($template->background_image)
                            <img src="{{ asset($template->background_image) }}" class="tpl-upload-preview" alt="Current background">
                            <span class="lbl">Click to replace background</span>
                        @else
        ...
        <div class="form-section {{ $step == 4 ? '' : 'd-none' }}">
            <div class="section-title mb-1">Edit the Content</div>
        ...
        <div class="form-footer tpl-wizard-footer">
            ...
            @else
                <button class="btn btn-secondary" type="button" onclick="history.back()">
                    <span class="material-icons-round" style="font-size:16px">arrow_back</span> Cancel
                </button>
            @endif

            @if($step < $totalSteps)
                <button class="btn btn-primary" type="button" wire:click="nextStep">
 ...
 * Lets the design-picker cards update Summernote synchronously on click,
 * exactly like insertPlaceholder() does for placeholder chips.
 */
 ...
 * straight into Summernote (if mounted) — the server-side selectDesign()
 * call (wired via wire:click on the card) is the actual source of truth
 * for certificate_content, so this works even before Step 4 is reached.
 */
...
(function loadSummernote() {
    ...
    function initWhenReady() {
        document.addEventListener('livewire:initialized', function () {
            ...
            // Keeps the visible editor in sync whenever the server sets
            // certificate_content via selectDesign() (Step 2 card clicks).
            Livewire.on('designContentUpdated', (event) => {
                var content = event.content ?? (Array.isArray(event) ? event[0]?.content : null);
                var el = $('#certificateContent');
                if (el.length && $(el).data('summernote-init') && content) {
                    el.summernote('code', content);
                }
            });

            // Fired when Step 4 is reached (or a failed save's offending
            // field was certificate_content) — mounts/re-syncs the editor.
            Livewire.on('ensureEditorSynced', (event) => {
                var content = event.content ?? (Array.isArray(event) ? event[0]?.content : null);
                var el = $('#certificateContent');
                if (!el.length) return;
                if (!$(el).data('summernote-init')) {
                    setTimeout(attachSummernote, 100);
                    setTimeout(function () {
                        if ($(el).data('summernote-init') && content) {
                            el.summernote('code', content);
                        }
                    }, 250);
                } else if (content && el.summernote('code') !== content) {
                    el.summernote('code', content);
                }
            });
