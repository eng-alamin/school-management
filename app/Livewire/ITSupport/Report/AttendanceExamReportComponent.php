...
class AttendanceExamReportComponent extends Component
{
    use WithPagination;
    ...
    public function updatingExamId(): void    { $this->resetPage(); }
    ...
    /**
     * Common filters (date range, exam/class/section/subject, search) ছাড়া status filter।
    ...
    public function render()
    {
        ...
        // Summary counts — একই ফিল্টার (তারিখ, exam/class/section/subject, search) দিয়ে, শুধু status বাদে
        $summary = $this->baseQueryWithoutStatus()
            ->select('status', DB::raw('count(*) as total'))
            ->groupBy('status')
            ->pluck('total', 'status');

        $exams = ExamSetup::where('institution_id', institution()->id)
            ->orderBy('name')
            ->get(['id', 'name']);
        ...
        return view('livewire.admin.report.attendance-exam-report-component', [
            'records'  => $records,
            'summary'  => $summary,
            'exams'    => $exams,
            'classes'  => $classes,
        ...
        ->layout('layouts.itsupport.app', [
            ...
            'breadcrumbs' => [
                ['name' => 'Exam Attendance Report', 'url' => route('itsupport.reports.attendances.exam')],
            ],
        ]);
    }
}